...
#define IsLChild(x) (!IsRoot(x) && (&(x)==(x.prarent->lc)))
#define IsRChild(x) (!IsRoot(x) && (&(x)==(x.prarent->rc)))
...
public:
	...
	int remove(BinNodePosi(T) x);				//删除以位置x处节点为根的子树，返回该子树原来的规模
	BinTree<T>* secede(BinNodePosi(T) x);		//将子树x从当前树种摘除，并将其转换为一颗独立的树
