...
batch_size_start = 500

batch_size_end = 50000
...
#----------------------------
    ...
    W1 = tf.Variable(tf.truncated_normal([num_of_inputs,num_of_nodes],0,0.1))
        
    b1 = tf.Variable(tf.truncated_normal([num_of_nodes]))
        
    h1 =  2 * tf.nn.sigmoid(tf.matmul(x,W1) + b1) - 1
        
    W2 = tf.Variable(tf.truncated_normal([num_of_nodes,num_of_nodes],0,10))
    ...
    train_step = tf.train.AdamOptimizer(learning_rate=0.1, beta1=0.95, beta2=0.99, epsilon=1e-08, use_locking=False).minimize(cross_entropy)
    #AdadeltaOptimizer
    #FtrlOptimizer(0.01)
    #RMSPropOptimizer
    #.GradientDescentOptimizer(0.00001)
